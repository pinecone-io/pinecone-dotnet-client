...
public class Helpers
{
    ...
    public static string GenerateIndexName(string testName)
    {
        ...
        if (testName.StartsWith("test_", StringComparison.OrdinalIgnoreCase)) testName = testName.Substring(5);

        // Trim name length to save space for other info in name
        testName = testName.Length > 20 ? testName.Substring(0, 20) : testName;

        // Remove trailing underscore, if any
        if (testName.EndsWith("_")) testName = testName.Substring(0, testName.Length - 1);
    ...
    public static string GetEnvironmentVar(string name, string? defaultVal = null)
    {
        return Environment.GetEnvironmentVariable(name) ??
               defaultVal ?? throw new Exception($"Expected environment variable {name} is not set");
    }

    public static void PollStatsForNamespace(IndexClient idx, string namespaceName, int expectedCount)
    {
        var maxSleep = int.TryParse(Environment.GetEnvironmentVariable("FRESHNESS_TIMEOUT_SECONDS"), out var timeout)
            ? timeout
            : 60;
        const int deltaT = 5;
        var totalTime = 0;
        ...
        while (!done)
        {
            ...
            if (stats.Namespaces.ContainsKey(namespaceName) &&
            ...
            {
                throw new TimeoutException($"Timed out waiting for namespace {namespaceName} to have vectors");
            }
            else
            {
                totalTime += deltaT;
    ...
    public static void PollFetchForIdsInNamespace(IndexClient idx, string[] ids, string namespaceName)
    {
        var maxSleep = int.TryParse(Environment.GetEnvironmentVariable("FRESHNESS_TIMEOUT_SECONDS"), out var timeout)
            ? timeout
            : 60;
        ...
        while (!done)
        {
            Console.WriteLine($"Attempting to fetch from \"{namespaceName}\". Total time waited: {totalTime} seconds");

            var results = idx.FetchAsync(new FetchRequest { Ids = ids, Namespace = namespaceName }).Result;

            Console.WriteLine(results);

            var allPresent = ids.All(id => results.Vectors.ContainsKey(id));
            if (allPresent) done = true;
    ...
    {
        return string.Join("-", RandomString(8), RandomString(4), RandomString(4), RandomString(4), RandomString(12));
    }
}